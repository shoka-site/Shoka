...
import { useIndustry, useIndustries } from "@/hooks/use-content";
...
export default function IndustryDetailsClient() {
            ...
            <Link href="/industries">
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <button className="bg-primary text-black hover:bg-white font-black tracking-widest uppercase transition-all shadow-xl flex gap-3 items-center rounded-full h-14 px-10 mx-auto">
                  ...
                  {t("home.industries.back", "Back to Industries")}
                </button>
              </motion.div>
            </Link>
          </motion.div>
  ...
  // Other industries (excluding current)
  const otherIndustries = allIndustries.filter((ind) => ind.id !== id).slice(0, 3);

  // Parse description paragraphs
  const descriptionParagraphs = (industry.description || "").split("\n").filter(Boolean);

  // Format date
  const formattedDate = industry.createdAt
    ? new Date(industry.createdAt).toLocaleDateString(i18n.language === "ar" ? "ar-IQ" : "en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
          ...
          >
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="mb-8">
              <Badge variant="outline" className="text-accent border-accent/40 bg-accent/10 px-5 py-2 text-xs backdrop-blur-md flex items-center justify-center gap-2 font-bold uppercase tracking-widest">
                ...
                {t("navbar.industries", "Industry")} · {String(industry.order).padStart(2, "0")}
              </Badge>
            </motion.div>
                  ...
                  <CardContent className="p-8 relative z-10 space-y-6">
                    <div>
                      <h3 className="text-xl font-display font-bold mb-2 text-foreground">
                        {t("home.industries.cta_sidebar.title", "Tailored for your Industry?")}
                      </h3>
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        {t("home.industries.cta_sidebar.desc", "Get customized digital solutions designed specifically for your industry vertical.")}
                      </p>
                    </div>
                    ...
                    >
                      {t("home.industries.view_all", "View All Industries")}
                    </Link>
                  </CardContent>
                </Card>
                  ...
                  <CardContent className="p-6 space-y-5">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("home.industries.order", "Industry Number")}</p>
                          <p className="text-sm font-semibold text-foreground">{String(industry.order).padStart(2, "0")}</p>
                        </div>
                      </div>
                      ...
                      {formattedDate && (
                        <>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("home.industries.added", "Added")}</p>
          ...
          <div className="container mx-auto">
            <FadeIn className="mb-12">
              <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground">
                {t("home.industries.other_industries", "Other Industries We Serve")}
              </h2>
              <p className="text-muted-foreground mt-2">{t("home.industries.other_industries_desc", "Discover how we support different business verticals.")}</p>
            </FadeIn>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {otherIndustries.map((ind, i) => (
                <FadeIn key={ind.id} delay={i * 0.08}>
                  <Link href={`/industries/${ind.id}`}>
                    <motion.div
                    ...
                    >
                      ...
                      {ind.description && (
                        <p className="text-muted-foreground text-sm leading-relaxed line-clamp-3 flex-1">{ind.description}</p>
                      )}
                      <span className={`mt-5 inline-flex items-center gap-1.5 text-primary text-sm font-semibold group-hover:underline ${isRtl ? "flex-row-reverse" : ""}`}>
                        {t("home.industries.learn_more", "Learn more")}
            ...
            <div className="max-w-3xl mx-auto p-10 md:p-14 rounded-[2.5rem] bg-muted/20 border border-border relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className={`flex flex-col sm:flex-row items-center justify-center gap-4 ${isRtl ? "sm:flex-row-reverse" : ""}`}>
                  ...
                  >
                    {t("navbar.industries", "All Industries")}
                  </Link>
                </div>
              </div>
            </div>
