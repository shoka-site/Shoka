...
import { useService, useServices } from "@/hooks/use-content";
...
export default function ServiceDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: service, isLoading } = useService(unwrappedParams.id);
      ...
      <div className="min-h-screen w-full flex flex-col items-center justify-center bg-black overflow-hidden relative">
        ...
        <div className="container relative z-10 mx-auto px-6 max-w-2xl text-center">
          ...
          >
            ...
            <h2 className="text-2xl md:text-4xl font-display font-bold text-white mb-6">{t("services.not_found", "Service not found")}</h2>
            <p className="text-white/50 text-base md:text-xl max-w-md mx-auto mb-12 font-light leading-relaxed">{t("services.not_found_desc", "The service you are looking for does not exist or has been removed.")}</p>
            <Link href="/services">
  ...
  const otherServices = allServices.filter((s) => s.id !== unwrappedParams.id).slice(0, 3);
  ...
  const formattedDate = service.createdAt
    ? new Date(service.createdAt).toLocaleDateString(i18n.language === "ar" ? "ar-IQ" : "en-US", {
        year: "numeric",
        month: "long",
        day: "numeric",
      })
          ...
          >
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }} className="mb-8">
              <Badge variant="outline" className="text-accent border-accent/40 bg-accent/10 px-5 py-2 text-xs backdrop-blur-md flex items-center justify-center gap-2 font-bold uppercase tracking-widest">
                ...
                {service.type || t("navbar.services", "Services")} · {String(service.order).padStart(2, "0")}
              </Badge>
            </motion.div>
                  ...
                  <CardContent className="p-8 relative z-10 space-y-6">
                    <div>
                      <h3 className="text-xl font-display font-bold mb-2 text-foreground">
                        {t("services.cta_sidebar.title", "Interested in this Service?")}
                      </h3>
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        {t("services.cta_sidebar.desc", "Contact us to start your journey and supercharge your business.")}
                      </p>
                    </div>
                  ...
                  <CardContent className="p-6 space-y-5">
                    ...
                    <div className="space-y-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("services.order", "Service Number")}</p>
                          <p className="text-sm font-semibold text-foreground">{String(service.order).padStart(2, "0")}</p>
                        </div>
                      </div>
                      ...
                      {formattedDate && (
                        <>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-xs text-muted-foreground uppercase tracking-wider">{t("services.added", "Added")}</p>
          ...
          <div className="container mx-auto">
            <FadeIn className="mb-12">
              <h2 className="text-2xl md:text-3xl font-display font-bold text-foreground">
                {t("services.other_services", "Other Services")}
              </h2>
              <p className="text-muted-foreground mt-2">{t("services.other_services_desc", "Explore more services we offer.")}</p>
            </FadeIn>
                ...
                <FadeIn key={s.id} delay={i * 0.08}>
                  <Link href={`/services/${s.id}`}>
                    ...
                    >
                      ...
                      {s.description && (
                        <p className="text-muted-foreground text-sm leading-relaxed line-clamp-3 flex-1">{s.description}</p>
                      )}
            ...
            <div className="max-w-3xl mx-auto p-10 md:p-14 rounded-[2.5rem] bg-muted/20 border border-border relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className={`flex flex-col sm:flex-row items-center justify-center gap-4 ${isRtl ? "sm:flex-row-reverse" : ""}`}>
                  ...
                  >
                    {t("navbar.services", "All Services")}
                  </Link>
                </div>
              </div>
            </div>
