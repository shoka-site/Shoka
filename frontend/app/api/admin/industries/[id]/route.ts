...
import { updateIndustrySchema } from '@shared/schema';
...
export const PUT = withErrorHandler(async (
  ...
  const updated = await storage.updateIndustry(id, parsed.data);
  if (!updated) return fail('NOT_FOUND', 'Industry not found', undefined, 404);
  return ok(updated);
});
...
export const DELETE = withErrorHandler(async (
  ...
  if (!deleted) return fail('NOT_FOUND', 'Industry not found', undefined, 404);
