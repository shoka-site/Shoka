...
export default function WelcomeV2() {
    ...
    // Parallax transforms
    const bgX = useTransform(springX, (val) => val * 20);
    const bgY = useTransform(springY, (val) => val * 20);
    ...
    return (
        <div className="h-screen w-full flex flex-col items-center justify-center bg-[#050505] text-white relative overflow-hidden">
            ...
            <motion.div
                ...
                style={{ scale: 1.1, x: bgX, y: bgY }}
            >
                <div className="absolute inset-0 bg-black/20 z-10"></div>
                <img
                    src={portal}
                    alt="Mesopotamian Portal"
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-black/40 z-30"></div>
            </motion.div>
            ...
            <div className="relative z-30 flex flex-col items-center text-center px-4">
                ...
                >
                    <div className="inline-block p-[1px] rounded-full bg-gradient-to-r from-transparent via-accent/50 to-transparent">
                        <div className="px-6 py-2 bg-black/50 backdrop-blur-md rounded-full border border-accent/10">
                            <span className={`text-accent text-xs uppercase ${isRtl ? 'tracking-normal' : 'tracking-[0.3em]'}`}>{t("welcome.badge")}</span>
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.p
                    ...
                    className="text-lg md:text-xl text-white/60 max-w-2xl font-light tracking-wide leading-relaxed mb-12"
                >
                    {t("welcome.subtitle")}
                    <br />
                ...
                >
                    <Link href="/home">
                        <a className="group relative inline-flex items-center gap-4 px-12 py-6 bg-accent/10 hover:bg-accent/20 border border-accent/30 hover:border-accent transition-all duration-500 rounded-sm overflow-hidden">
