...
import type { ProcessStep } from "@/hooks/use-content";
...
export default function AdminProcessSteps() {
    ...
    const { data: steps = [], isLoading } = useQuery<ProcessStep[]>({
        ...
        queryFn: async () => {
            const res = await fetch("/api/content/process-steps?lang=en");
            return res.json();
        },
    });
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id: string) => {
            await fetch(`/api/admin/process-steps/${id}`, { method: "DELETE" });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["admin-process-steps"] });
            queryClient.invalidateQueries({ queryKey: ["processSteps"] });
    ...
    const handleEdit = (step: any) => {
        setFormData({
            ...
            titleAr: step.titleAr || step.title,
            descriptionEn: step.descriptionEn || step.description,
            descriptionAr: step.descriptionAr || step.description,
            order: step.order,
        ...
        <AdminLayout>
            <div className="max-w-6xl">
                ...
                <div className="space-y-4">
                    {steps.map((step: any) => (
                        <div key={step.id} className="p-6 bg-muted rounded-lg flex justify-between items-start">
                            <div className="flex gap-4 flex-1">
                                ...
                                <div className="flex-1">
                                    ...
                                    <h3 className="text-xl font-bold mb-2">{step.title || step.titleEn}</h3>
                                    <p className="text-sm">{step.description || step.descriptionEn}</p>
                                </div>
                            </div>
