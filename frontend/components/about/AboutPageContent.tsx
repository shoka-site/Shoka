...
import type { TeamMember, PlatformUpdate } from "@shared/schema";
...
export function AboutPageContent({ teamMembers, updates }: Props) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 md:gap-20 items-center">
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <div className="space-y-6 mb-10 md:mb-12">
                <p className="text-lg md:text-xl text-muted-foreground font-light leading-relaxed italic border-l-4 border-primary/20 pl-4 md:pl-6">{t("about.legacy_p1")}</p>
                <p className="text-base md:text-lg text-muted-foreground font-light leading-relaxed">{t("about.legacy_p2")}</p>
              </div>
              ...
              <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} viewport={{ once: true }}>
                <Card className="h-full bg-background border border-border/50 hover:border-primary/30 rounded-[2rem] shadow-sm hover:shadow-xl transition-all duration-700">
                  <CardHeader className="pb-4">
                    <div className="text-primary mb-6 p-4 bg-primary/5 w-fit rounded-2xl border border-primary/10">{val.icon}</div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {awards.length > 0 ? awards.map((award, index) => (
              ...
              >
                ...
                <h3 className="font-black text-2xl mb-3 tracking-tight font-display">{pick(award.titleEn, award.titleAr)}</h3>
                <p className="text-sm text-muted-foreground font-light px-4 leading-relaxed">{pick(award.summaryEn, award.summaryAr)}</p>
              </motion.div>
            )) : [1, 2, 3, 4].map((_, i) => (
            ...
            {teamMembers.map((member, index) => (
              ...
              >
                <div className="group relative overflow-hidden rounded-[2.5rem] bg-background border border-border/50 hover:border-primary/40 transition-all duration-700 h-full flex flex-col shadow-sm hover:shadow-2xl">
                  <div className="aspect-[3/4] overflow-hidden grayscale group-hover:grayscale-0 transition-all duration-1000 ease-out relative">
                    <Image
                      src={member.imageUrl}
                  ...
                  <div className="p-8 pt-0 relative -mt-20 z-10 flex flex-col flex-grow">
                    <h3 className="text-2xl font-black font-display text-foreground group-hover:text-primary transition-colors tracking-tight leading-tight">
                      {pick(member.nameEn, member.nameAr)}
                    </h3>
                    <p className="text-xs font-bold text-accent mb-6 tracking-[0.2em] uppercase">
                      {pick(member.roleEn, member.roleAr)}
                    </p>
                    <p className="text-sm text-muted-foreground font-light leading-relaxed line-clamp-4">
                      {pick(member.bioEn, member.bioAr)}
                    </p>
